...
export default function AppShell({ children }) {
  ...
  `}</style><div className="app-shell"><div className="app-header"><a href={shopUrl} style={{display:'flex',alignItems:'center',gap:8,textDecoration:'none'}} aria-label="Protlys store"><img src="/protlys-logo.svg" alt="Protlys" style={{height:36,width:'auto',objectFit:'contain'}} /></a><a className="shop-header-link" href={shopUrl} aria-label="Shop Protlys" title="Shop Protlys"><CartIcon size={18}/><span>Shop Protlys</span></a></div><section className="screen active">{children}</section><div className="bottom-nav">{NAV_ITEMS.map(item=><Link key={item.href} href={item.href} className={`nav-btn${pathname===item.href?' active':''}`}>{item.icon}{item.label}</Link>)}</div></div></div>;
}
