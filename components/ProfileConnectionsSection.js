...
function Avatar({ name, url }) {
  ...
  if (url) return <img src={url} alt="" loading="lazy" style={style} />;
...
function List({ people, loading, title }) {
  if (loading) return <div style={{display:'grid',gap:8}}>{[1,2,3].map(i=><div key={i} style={{height:64,borderBottom:'1px solid var(--line)',background:'linear-gradient(90deg,var(--paper),#fff,var(--paper))',borderRadius:8}} />)}</div>;
...
export default function ProfileConnectionsSection({ profileId, followerCount = 0, followingCount = 0 }) {
  ...
  const scrollToType = (nextType) => {
    setType(nextType);
    load(nextType);
    const index = nextType === 'followers' ? 0 : 1;
    sliderRef.current?.scrollTo({ left:index * sliderRef.current.clientWidth, behavior:'smooth' });
  };

  useEffect(() => {
    const handler = event => scrollToType(event.detail?.type === 'following' ? 'following' : 'followers');
    window.addEventListener('protlys-profile-connections', handler);
    return () => window.removeEventListener('protlys-profile-connections', handler);
  }, [load]);

  useEffect(() => {
    // Warm both lists after the profile has painted. This keeps initial profile
    // rendering light while making a later swipe feel immediate.
  ...
  return <section id="connections" style={{flex:'0 0 100%',minWidth:0,scrollSnapAlign:'start',scrollMarginTop:140,paddingTop:8,paddingBottom:24}}>
    ...
    <div ref={sliderRef} onScroll={() => { const slider=sliderRef.current; if(!slider)return; const next=slider.scrollLeft > slider.clientWidth/2 ? 'following' : 'followers'; if(next!==type){setType(next);load(next);} }} style={{display:'flex',overflowX:'auto',scrollSnapType:'x mandatory',scrollBehavior:'smooth',scrollbarWidth:'none',overscrollBehaviorX:'contain'}}>
