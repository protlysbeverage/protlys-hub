...
export default function ProfileConnectionsSection({ profileId, followerCount = 0, followingCount = 0 }) {
  ...
  const handlePointerMove = useCallback((event) => {
    ...
    if (!drag.active) {
      ...
      // Let the browser/page handle vertical movement normally. Only take
      // over once the gesture is clearly horizontal.
  ...
  const finishPointer = useCallback((event) => {
    ...
    if (Math.abs(dx) >= threshold) {
      switchType(dx < 0 ? 'following' : 'followers');
    } else {
      setDragX(0);
    }
  ...
  return <section id="connections" style={{flex:'0 0 100%',minWidth:0,scrollSnapAlign:'start',scrollMarginTop:140,paddingTop:8,paddingBottom:24}}>
    ...
    <div
      ...
      style={{width:'100%',overflow:'hidden',touchAction:'pan-y',userSelect:'none',cursor:dragging?'grabbing':'grab'}}
    >
