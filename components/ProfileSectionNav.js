...
export default function ProfileSectionNav({ profileId }) {
  ...
  return (
    <nav aria-label="Profile sections" style={{display:'grid',gridTemplateColumns:'repeat(4,minmax(0,1fr))',gap:2,margin:'12px 0 5px',padding:3,background:'#fff',border:'1px solid var(--line)',borderRadius:12,position:'sticky',top:76,zIndex:10,boxShadow:'0 2px 7px rgba(0,0,0,.035)'}}>
      {sections.map(([id,label])=><button key={id} type="button" onClick={(event)=>goToSection(event,id)} aria-current={!onConnectionsPage&&active===id?'page':undefined} style={{minWidth:0,border:0,textAlign:'center',padding:'9px 2px',borderRadius:9,background:!onConnectionsPage&&active===id?'var(--green-soft)':'transparent',color:!onConnectionsPage&&active===id?'var(--green-dark)':'var(--ink-45)',fontSize:10,fontWeight:800,whiteSpace:'nowrap',transition:'background .18s ease,color .18s ease',cursor:'pointer'}}>{label}</button>)}
