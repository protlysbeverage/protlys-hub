...
// Isolated strength bar — only re-renders when password length bracket changes
      ...
      <div style={{ display: 'flex', gap: 4, marginBottom: 4 }}>
        {[0,1,2].map(i => (
          <div key={i} style={{ flex:1, height:4, borderRadius:99,
            background: i < score ? colors[score] : 'var(--ink-20)', transition:'background .25s' }} />
        ))}
      </div>
...
export default function SignupPage() {
  ...
  // Only update strength state — input itself is uncontrolled
  ...
  async function handleSubmit(e) {
    ...
    const { error } = await supabase.auth.signUp({
      email, password: pw,
      options: { data: { display_name: nameRef.current.value } },
    });
    setLoading(false);
      ...
      <div className="app-shell" style={{ justifyContent:'flex-start', overflowY:'auto' }}>
        <div className="screen-pad" style={{ paddingTop:40 }}>
          <div style={{ textAlign:'center', marginBottom:32 }}>
            <img src="/logo.png" alt="Protlys" style={{ width:120, height:'auto' }} onError={e=>{e.target.style.display='none'}} />
          </div>
          <div style={{ textAlign:'center' }}>
            <div style={{ fontSize:52, marginBottom:12 }}>🎉</div>
            <h1 style={{ fontSize:22 }}>You're almost in!</h1>
            <p className="subhead" style={{ marginTop:8 }}>
              Confirmation link sent to <strong>{doneEmail}</strong>. Click it to activate your account.
            </p>
            <div style={{ background:'var(--green-soft)', borderRadius:14, padding:14, marginTop:16 }}>
    ...
    <div className="protlys-app">
      <div className="app-shell" style={{ justifyContent:'flex-start', overflowY:'auto' }}>
        <div className="screen-pad" style={{ paddingTop:40, paddingBottom:40 }}>
          ...
          <form onSubmit={handleSubmit}>
            <div className="field-group">
              ...
              <input id="name" ref={nameRef} type="text" required
                className="field-input" placeholder="John Doe" autoComplete="name" />
            </div>
            <div className="field-group">
              <label className="field-label" htmlFor="email">EMAIL</label>
              <input id="email" ref={emailRef} type="email" required
                className="field-input" placeholder="you@example.com" autoComplete="email" />
            </div>
            <div className="field-group">
              ...
              <div style={{ position:'relative' }}>
                <input id="password" ref={passwordRef} type="password" required
                  ...
                  autoComplete="new-password" onChange={handlePwChange}
            ...
            <div style={{ display:'flex', alignItems:'flex-start', gap:10, marginBottom:16 }}>
              ...
              <label htmlFor="agree" style={{ fontSize:13, color:'var(--ink-70)', lineHeight:1.5, cursor:'pointer' }}>
                ...
                <a href="https://protlys.com/pages/terms" style={{ color:'var(--green-dark)', fontWeight:700 }}>Terms</a>
                {' '}and{' '}
                <a href="https://protlys.com/pages/privacy" style={{ color:'var(--green-dark)', fontWeight:700 }}>Privacy Policy</a>
              </label>
            </div>
          ...
          <p style={{ textAlign:'center', marginTop:18, fontSize:14, color:'var(--ink-70)' }}>
...
const googleBtnStyle = {
  ...
  background:'#fff', border:'1.5px solid var(--line)', borderRadius:12, padding:'13px 16px',
  fontSize:14, fontWeight:600, cursor:'pointer', color:'var(--ink)', marginTop:20, fontFamily:'inherit',
};
