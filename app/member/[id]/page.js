...
import ProfileConnectionLauncher from '@/components/ProfileConnectionLauncher';
...
export default async function MemberProfilePage({params}){
  ...
  const [{data:profile},{data:posts},{count:postCount},{data:achievements},{data:movementDays},{data:movementPosts},{count:followerCount},{count:followingCount},{data:followRow}]=await Promise.all([
    supabase.from('profiles').select('id, display_name, avatar_url, bio, created_at, streak, step_streak, total_steps, step_goal, target_g').eq('id',id).maybeSingle(),
    supabase.from('feed_posts').select('id, body, image_url, post_type, stats, created_at, feed_likes(count), feed_comments(count)').eq('user_id',id).order('created_at',{ascending:false}).limit(20),
    ...
    supabase.from('feed_posts').select('created_at, stats').eq('user_id',id).not('stats','is',null).order('created_at',{ascending:true}),
    supabase.from('follows').select('id',{count:'exact',head:true}).eq('following_id',id),
    supabase.from('follows').select('id',{count:'exact',head:true}).eq('follower_id',id),
    supabase.from('follows').select('id').eq('follower_id',user.id).eq('following_id',id).maybeSingle(),
  ]);
  if(!profile)notFound();
  // daily_steps is the authoritative movement ledger. For older feed posts created
  // before feed-to-movement syncing existed, fill only dates that have no ledger row.
  // This keeps public profiles consistent without double-counting days already synced.
  ...
  const publicMovementDays=[...movementByDate.entries()]
    .map(([step_date,steps])=>({step_date,steps}))
    .sort((a,b)=>a.step_date.localeCompare(b.step_date));
  const publicTotalSteps=publicMovementDays.reduce((sum,row)=>sum+Number(row.steps||0),0);
  const displayTotalSteps=publicTotalSteps>0?publicTotalSteps:Number(profile.total_steps||0);
  const displayStepStreak=publicMovementDays.filter(row=>Number(row.steps||0)>0).length
    ...
    <section style={{background:'#fff',border:'1.5px solid var(--line)',borderRadius:20,padding:'16px 14px 14px',boxShadow:'0 2px 8px rgba(0,0,0,.035)'}}>
      ...
      <div style={{marginTop:13,borderTop:'1px solid var(--line)',borderBottom:'1px solid var(--line)',padding:'10px 0'}}>
        <div style={{display:'grid',gridTemplateColumns:'repeat(3,minmax(0,1fr))'}}>
          ...
          <ProfileConnectionLauncher profileId={id} type="followers" count={followerCount||0} borderRight />
          <ProfileConnectionLauncher profileId={id} type="following" count={followingCount||0} />
        </div>
      </div>
      {bio&&<div style={{paddingTop:11,fontSize:13,lineHeight:1.45,color:'var(--ink-70)',whiteSpace:'pre-wrap',overflowWrap:'anywhere'}}>{bio}</div>}
      <div style={{fontSize:10.5,color:'var(--ink-45)',fontWeight:700,marginTop:bio?8:10}}>Joined {formatJoinedDate(profile.created_at)}</div>
    </section>
    <ProfileSectionNav profileId={id}/>
    <ProfileSectionSlider>
      <section id="posts" style={{flex:'0 0 100%',minWidth:0,scrollSnapAlign:'start',scrollMarginTop:140,paddingTop:8,paddingBottom:6}}><div className="eyebrow" style={{marginBottom:8}}>Posts</div>{posts?.length?posts.map((post,index)=><article key={post.id} className="profile-post-card" style={{background:'#fff',border:'1.5px solid var(--line)',borderRadius:16,padding:14,marginBottom:10}}><div style={{display:'flex',justifyContent:'space-between',gap:10,marginBottom:7}}><span style={{fontSize:10.5,color:'var(--ink-45)'}}>{formatNairobiDate(post.created_at)}</span>{post.post_type&&post.post_type!=='general'&&<span style={{background:'var(--green-soft)',color:'var(--green-dark)',padding:'2px 7px',borderRadius:999,fontSize:10,fontWeight:800}}>{post.post_type.replace('_',' ')}</span>}</div>{post.body&&<p className="post-content-text" style={{fontSize:13.5,lineHeight:1.55,margin:'0 0 10px',whiteSpace:'pre-wrap',overflowWrap:'break-word'}}>{post.body}</p>}<PostStats stats={post.stats}/>{post.image_url&&<img src={post.image_url} alt="" loading={index===0?'eager':'lazy'} decoding="async" style={{width:'100%',maxHeight:280,objectFit:'cover',borderRadius:12,display:'block',marginTop:post.stats?10:0}}/>}<MemberPostComments postId={post.id} initialCount={post.feed_comments?.[0]?.count||0} initialLikeCount={post.feed_likes?.[0]?.count||0}/></article>):<div style={{background:'#fff',border:'1.5px solid var(--line)',borderRadius:16,padding:'28px 18px',textAlign:'center'}}><div style={{fontWeight:800}}>No posts yet</div><p className="subhead" style={{margin:'5px 0 0'}}>This member has not shared anything to the feed.</p></div>}</section>
      <section id="stats" style={{flex:'0 0 100%',minWidth:0,scrollSnapAlign:'start',scrollMarginTop:140,paddingTop:8,paddingBottom:6}}><div className="eyebrow" style={{marginBottom:8}}>Stats & movement</div><div style={{display:'grid',gridTemplateColumns:'repeat(2,1fr)',gap:8,marginBottom:12}}><div style={{background:'#fff',border:'1.5px solid var(--line)',borderRadius:14,padding:14}}><div style={{fontSize:9,textTransform:'uppercase',letterSpacing:'.06em',fontWeight:800,color:'var(--ink-45)'}}>Step streak</div><div className="mono" style={{fontSize:22,fontWeight:800,marginTop:4}}>{displayStepStreak}</div></div><div style={{background:'#fff',border:'1.5px solid var(--line)',borderRadius:14,padding:14}}><div style={{fontSize:9,textTransform:'uppercase',letterSpacing:'.06em',fontWeight:800,color:'var(--ink-45)'}}>Total steps</div><div className="mono" style={{fontSize:22,fontWeight:800,marginTop:4}}>{displayTotalSteps.toLocaleString()}</div></div></div><MovementActivity days={publicMovementDays} title={`${displayName}'s movement`}/>{achievements?.length>0&&<section style={{marginTop:14}}><div className="eyebrow" style={{marginBottom:8}}>Milestones</div><div style={{display:'flex',gap:8,overflowX:'auto',paddingBottom:2}}>{achievements.map(item=><div key={`${item.achievements?.slug}-${item.earned_at}`} style={{flex:'0 0 auto',border:'1px solid var(--line)',borderRadius:12,padding:'10px 12px',background:'#fff',minWidth:150}}><div style={{fontSize:12,fontWeight:800}}>{item.achievements?.name||'Milestone'}</div>{item.achievements?.description&&<div style={{fontSize:10.5,color:'var(--ink-45)',marginTop:3}}>{item.achievements.description}</div>}</div>)}</div></section>}</section>
      <section id="photos" style={{flex:'0 0 100%',minWidth:0,scrollSnapAlign:'start',scrollMarginTop:140,paddingTop:8,paddingBottom:24}}><div className="eyebrow" style={{marginBottom:8}}>Photos</div>{photoPosts.length?<div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:5}}>{photoPosts.map(post=><a key={post.id} href={`#photo-${post.id}`} style={{display:'block',aspectRatio:'1 / 1',overflow:'hidden',borderRadius:10,background:'var(--paper)'}}><img src={post.image_url} alt="" loading="lazy" decoding="async" style={{width:'100%',height:'100%',objectFit:'cover',display:'block'}}/></a>)}</div>:<div style={{background:'#fff',border:'1.5px solid var(--line)',borderRadius:16,padding:'28px 18px',textAlign:'center'}}><div style={{fontWeight:800}}>No photos yet</div><p className="subhead" style={{margin:'5px 0 0'}}>Photos shared by this member will appear here.</p></div>}</section>
    </ProfileSectionSlider>
  </div></AppShell>;
}
