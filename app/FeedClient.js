...
function Avatar({ name, url, size = 40, href }) {
  const avatar = url ? <img src={url} alt="" style={{ width: size, height: size, borderRadius: '50%', objectFit: 'cover', flexShrink: 0 }} /> : <div style={{ width: size, height: size, borderRadius: '50%', background: 'var(--green-soft)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: size * 0.38, fontWeight: 800, color: 'var(--green-dark)', flexShrink: 0 }}>{(name || '?')[0].toUpperCase()}</div>;
...
function CommentInput({ postId, onDone }) {
  ...
  async function submit() { const body = ref.current?.value?.trim(); if (!body || loading) return; setLoading(true); const result = await addFeedCommentAction({ postId, body }); setLoading(false); if (result?.error) return; ref.current.value = ''; onDone(); }
...
export default function FeedClient({ posts, likedIds, userId, profile }) {
  const router = useRouter(); const [liked, setLiked] = useState(new Set(likedIds)); const [commenting, setCommenting] = useState(null); const [toast, setToast] = useState('');
  ...
  async function handleLike(postId) { const wasLiked = liked.has(postId); setLiked(current => { const next = new Set(current); if (next.has(postId)) next.delete(postId); else next.add(postId); return next; }); const result = await toggleFeedLikeAction({ postId, like: !wasLiked }); if (result?.error) { setLiked(current => { const next = new Set(current); if (wasLiked) next.add(postId); else next.delete(postId); return next; }); showToast(result.error); return; } router.refresh(); }
  ...
  return <div className="screen-pad"><span className="eyebrow">Community</span><h1>Progress feed</h1><p className="subhead">Share movement, milestones and everyday wins with the Hub.</p><ComposePost profile={profile} onPosted={() => { showToast('Posted'); router.refresh(); }} />{posts?.length ? posts.map(post => <article key={post.id} className="feed-card"><div className="feed-card-head"><Avatar name={post.profiles?.display_name} url={post.profiles?.avatar_url} href={`/member/${post.user_id}`} /><div style={{ flex: 1, minWidth: 0 }}><Link href={`/member/${post.user_id}`} className="feed-author" aria-label={`View ${post.profiles?.display_name || 'member'} profile`}>{post.profiles?.display_name || 'Member'}</Link><div className="feed-meta">{timeAgo(post.created_at)}</div></div><Link href={`/member/${post.user_id}`} aria-label={`View ${post.profiles?.display_name || 'member'} stats`} className="btn-secondary" style={{ width: 'auto', padding: '6px 9px', marginTop: 0, fontSize: 10.5, textDecoration: 'none', whiteSpace: 'nowrap' }}>Profile</Link></div>{post.body && <div className="feed-body">{post.body}</div>}{post.image_url && <img className="feed-image" src={post.image_url} alt="" />}{post.stats && <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: 12 }}>{post.stats.steps && <StatChip label="Steps" value={post.stats.steps} />}{post.stats.distance && <StatChip label="Distance" value={`${post.stats.distance} km`} />}{post.stats.duration && <StatChip label="Duration" value={post.stats.duration} />}</div>}<div className="feed-actions"><button type="button" onClick={() => handleLike(post.id)} aria-label={liked.has(post.id) ? 'Unlike post' : 'Like post'}><Icon name="heart" size={19} strokeWidth={liked.has(post.id) ? 2.3 : 1.8} /> <span>{post.like_count || 0}</span></button><button type="button" onClick={() => setCommenting(commenting === post.id ? null : post.id)}><Icon name="message" size={19} /> <span>{post.comment_count || 0}</span></button><button type="button" onClick={() => handleShare(post)} aria-label="Share post"><Icon name="share" size={19} /> <span>Share</span></button></div>{commenting === post.id && <CommentInput postId={post.id} onDone={refreshComments} />}</article>) : <EmptyState />}{toast && <div style={{ position: 'fixed', left: '50%', bottom: 84, transform: 'translateX(-50%)', background: 'var(--ink)', color: '#fff', padding: '9px 14px', borderRadius: 999, fontSize: 12, fontWeight: 700, zIndex: 30 }}>{toast}</div>}</div>;
}
