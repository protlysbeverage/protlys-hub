...
function ComposePost({ profile, onPosted }) {
  ...
  return <div className="feed-composer" style={{ background: '#fff', borderRadius: 20, padding: 16, marginBottom: 16, border: '1.5px solid var(--line)', boxShadow: '0 2px 8px rgba(15,42,74,.04)' }}>
    ...
    <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', gap:8, minWidth:0 }}><div style={{display:'flex',alignItems:'center',gap:2,minWidth:0}}><button type="button" onClick={()=>fileRef.current?.click()} aria-label="Add photo" title="Add photo" style={{width:44,height:44,border:0,background:'transparent',borderRadius:11,color:'var(--ink-70)',display:'grid',placeItems:'center',cursor:'pointer',flexShrink:0}}><Icon name="image" size={19}/></button><button type="button" onClick={()=>setShowStats(value=>!value)} aria-label="Add activity" title="Add activity" aria-pressed={showStats} style={{width:44,height:44,border:0,background:showStats?'var(--green-soft)':'transparent',borderRadius:11,color:showStats?'var(--green-dark)':'var(--ink-70)',display:'grid',placeItems:'center',cursor:'pointer',flexShrink:0}}><Icon name="chart" size={19}/></button><div ref={typeMenuRef} style={{position:'relative',minWidth:0,zIndex:30}}><button type="button" onClick={()=>setShowTypeMenu(value=>!value)} aria-haspopup="listbox" aria-expanded={showTypeMenu} style={{minHeight:44,maxWidth:'min(145px,42vw)',border:'1px solid var(--line)',background:'var(--paper)',borderRadius:12,padding:'0 9px',display:'inline-flex',alignItems:'center',gap:6,color:'var(--ink)',fontFamily:'inherit',fontSize:12,fontWeight:800,cursor:'pointer',whiteSpace:'nowrap'}}><span style={{display:'inline-flex',color:'var(--green-dark)'}}><Icon name={postType==='run'?'chart':'image'} size={15}/></span><span style={{overflow:'hidden',textOverflow:'ellipsis'}}>{postTypeLabel(postType)}</span><Icon name="chevron" size={14}/></button>{showTypeMenu&&<div role="listbox" aria-label="Post category" style={{position:'absolute',top:'calc(100% + 6px)',left:0,zIndex:1000,width:'min(190px, calc(100vw - 40px))',boxSizing:'border-box',background:'var(--card)',border:'1px solid var(--line)',borderRadius:14,padding:5,boxShadow:'0 14px 32px rgba(15,42,74,.18)',overflow:'hidden'}}>{POST_TYPES.map(type=><button key={type.value} type="button" role="option" aria-selected={postType===type.value} onClick={()=>{setPostType(type.value);setShowTypeMenu(false)}} style={{width:'100%',minHeight:44,border:0,borderRadius:10,background:postType===type.value?'var(--green-soft)':'transparent',color:'var(--ink)',display:'flex',alignItems:'center',gap:9,padding:'0 10px',fontFamily:'inherit',fontSize:12,fontWeight:700,textAlign:'left',cursor:'pointer'}}><span style={{display:'inline-flex',color:'var(--green-dark)'}}><Icon name={type.value==='run'?'chart':'image'} size={16}/></span><span style={{flex:1}}>{type.label}</span>{postType===type.value&&<Icon name="check" size={16}/>}</button>)}</div>}</div></div><button className="btn-primary" onClick={handlePost} disabled={loading} style={{ width:'auto', minHeight:44, padding:'0 16px', marginTop:0, flexShrink:0 }}>{loading ? 'Posting…' : 'Post'}</button></div>
