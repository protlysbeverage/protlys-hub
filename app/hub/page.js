...
export default async function HubPage() {
  ...
  if (!user) {
    const { data: foundingCount } = await supabase.rpc('get_founding_member_count');
    return <div className="protlys-app"><div className="app-shell" style={{display:'block',minHeight:'100vh'}}><PublicHub foundingCount={foundingCount || 0} /></div></div>;
  }

  const today = nairobiDateStr();
  const [{ data: profile }, { data: logs }, { data: movementDays }, { data: foundingCount }] = await Promise.all([
    ...
    supabase.rpc('get_founding_member_count'),
  ]);
  ...
  return (
    <AppShell>
      <HubClient
        profile={profile || { display_name:user.email, target_g:120, onboarding_complete:false }}
        todayG={todayG}
        logs={logs || []}
        movementDays={movementDays || []}
        foundingCount={foundingCount || 0}
      />
    </AppShell>
  );
}
