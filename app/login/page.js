...
// Isolated input component — prevents parent re-renders from stealing focus
function Field({ id, label, type, inputRef, placeholder, autoComplete, children }) {
  return (
    <div className="field-group">
      <label className="field-label" htmlFor={id}>{label}</label>
      <div style={{ position: 'relative' }}>
        <input id={id} ref={inputRef} type={type} required
          className="field-input" placeholder={placeholder}
          autoComplete={autoComplete} style={{ paddingRight: children ? 48 : undefined }} />
        {children}
      </div>
    </div>
  );
...
export default function LoginPage() {
  ...
  // Update password input type without re-rendering (swap type attribute directly)
  function togglePw() {
    setShowPw(v => {
      const next = !v;
        ...
        <div className="screen-pad" style={{ paddingTop: 40 }}>
          ...
          <div style={{ textAlign: 'center' }}>
            ...
            <p className="subhead" style={{ marginTop: 8 }}>
              Reset link sent to <strong>{resetSentTo}</strong>. Check spam if you don't see it.
            </p>
    ...
    <div className="protlys-app">
      <div className="app-shell" style={{ justifyContent: 'flex-start', overflowY: 'auto' }}>
        <div className="screen-pad" style={{ paddingTop: 40 }}>
          ...
          <p className="subhead" style={{ marginTop: 6 }}>We'll send a reset link to your email.</p>

          <form onSubmit={handleForgot} style={{ marginTop: 20 }}>
            <div className="field-group">
              <label className="field-label" htmlFor="resetEmail">EMAIL</label>
              <input id="resetEmail" ref={resetRef} type="email" required
...
const googleBtnStyle = {
  ...
  color:'var(--ink)', marginTop:20, fontFamily:'inherit',
};
const dividerStyle = {
  display:'flex', alignItems:'center', gap:12, margin:'18px 0',
  color:'var(--ink-45)', fontSize:13, fontWeight:600,
};
