...
export default async function FeedPage() {
  ...
  const normalizedPosts = (posts || []).map(post => ({...post,like_count:post.feed_likes?.[0]?.count||0,comment_count:post.feed_comments?.[0]?.count||0,profiles:post.profiles||{id:post.user_id,display_name:null,avatar_url:null}}));

  return <AppShell><FeedVisualPatch/><FeedClient posts={normalizedPosts} likedIds={[...likedIds]} userId={user.id} profile={profile||{}}/><FeedInteractionEnhancer posts={normalizedPosts}/></AppShell>;
}
